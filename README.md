...
The default settlement token is Base USDC (`chainId=8453`, token `0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913`). Use `wrangler secret put KIRAPAY_API_KEY`, `wrangler secret put KIRAPAY_WEBHOOK_SECRET`, and `wrangler secret put ORGANIZER_PASSCODE` for sensitive values. `KIRAPAY_WEBHOOK_URL` is only needed locally when running `npm run kirapay:webhook:register`; it is not read by the Worker at runtime.
...
It also expects these GitHub **Production environment secrets**:
...
On deploy, the workflow writes the KIRAPAY and organizer secrets into Cloudflare Worker secrets, deploys the Worker, and registers the KIRAPAY webhook endpoint.
